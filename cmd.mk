...
flash: $(BUILD_DIR)/$(TARGET).hex
	...
	openocd -f $(SDK_DIR)/scripts/openocd/stlink-v2.cfg -f $(SDK_DIR)/scripts/openocd/hk32f1x.cfg -c init -c targets -c 'reset halt' -c 'flash write_image erase $(BUILD_DIR)/$(TARGET).hex' -c 'reset halt' -c 'verify_image $(BUILD_DIR)/$(TARGET).hex' -c 'reset run' -c shutdown
